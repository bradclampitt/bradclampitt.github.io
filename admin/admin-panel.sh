...
ADMIN_ROOT="/var/www/projectmanager.test/github_v2/admin"
...
find_process() {
    ...
    if [ -n "$port_pid" ]; then
        ...
        if ps -p "$port_pid" -o args= | grep -q "uvicorn.*app:app"; then
            echo "$port_pid"
            return 0
        fi
    fi
...
check_status() {
    ...
    if [ -n "$pid" ]; then
        ...
        if command -v ps > /dev/null; then
            ...
            # Use simple ps command that works on most systems
            ps -p "$pid" 2>/dev/null | head -n2 || echo "  (Unable to display process details)"
        fi
